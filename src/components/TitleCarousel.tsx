 ...
 */
export function TitleCarousel({ titles, equippedAchievementId, onEquip, equipping }: TitleCarouselProps) {
  ...
  useEffect(() => {
    if (equippedAchievementId) {
      const equippedIndex = titles.findIndex(t => t.achievementId === equippedAchievementId);
      if (equippedIndex !== -1) {
  ...
  const handleEquip = async () => {
    if (!selectedTitle || !selectedTitle.isUnlocked || equipping || isAnimating) return;
  ...
  const shareToSocial = (platform: string) => {
    ...
    const shareUrl = 'https://www.erastimecapsule.com';
    ...
    switch (platform) {
      ...
      case 'copy':
        ...
        toast.success('Title details copied to clipboard!', { icon: '📋', duration: 2000 });
      ...
      case 'native':
        if (navigator.share) {
          navigator.share({
            ...
            navigator.clipboard.writeText(`${shareText}\n\n${shareUrl}`);
            toast.success('Share text copied to clipboard!', { icon: '📋', duration: 2000 });
          });
        }
        setShowShareMenu(false);
        return;
  ...
  const badgeConfig = selectedTitle ? getBadgeGeometry(selectedTitle.rarity, selectedTitle.title) : null;
    ...
    >
                  ...
                  <div className="relative">
                    ...
                    <motion.div
                      ...
                      style={{
                        ...
                        boxShadow: isSelected && title.isUnlocked 
                      ...
                      animate={
                        isSelected && title.isUnlocked ? (
                          titleConfig.animation === 'clock-tick' ? { rotate: [0, 5, 0] } :
                          titleConfig.animation === 'camera-flash' ? { scale: [1, 1.05, 1] } :
                          titleConfig.animation === 'star-twinkle' ? { scale: [1, 1.1, 1], rotate: [0, 180, 360] } :
                          titleConfig.animation === 'curtain-sway' ? { x: [-1, 1, -1] } :
                          titleConfig.animation === 'page-turn' ? { rotateY: [0, 10, 0] } :
                          titleConfig.animation === 'thread-weave' ? { x: [-2, 2, -2], y: [-1, 1, -1] } :
                          titleConfig.animation === 'anvil-strike' ? { y: [0, -2, 0], scale: [1, 1.05, 1] } :
                          titleConfig.animation === 'wave-pulse' ? { scaleY: [1, 1.1, 1] } :
                          titleConfig.animation === 'ripple-out' ? { scale: [1, 1.08, 1] } :
                          titleConfig.animation === 'blueprint-scan' ? { opacity: [1, 0.8, 1] } :
                          titleConfig.animation === 'scroll-unfurl' ? { scaleX: [1, 1.05, 1] } :
                          titleConfig.animation === 'crown-sparkle' ? { scale: [1, 1.1, 1], rotate: [0, -5, 0] } :
                          titleConfig.animation === 'book-glow' ? { filter: ['brightness(1)', 'brightness(1.2)', 'brightness(1)'] } :
                          titleConfig.animation === 'supernova' ? { scale: [1, 1.2, 1], rotate: [0, 360, 720] } :
                          titleConfig.animation === 'vortex-spin' ? { rotate: [0, 360] } :
                          { scale: [1, 1.05, 1] }
                        ) : {}
                      }
                      transition={{ 
                        duration: titleConfig.intensity === 'supreme' ? 2 : titleConfig.intensity === 'high' ? 2.5 : 3,
                    ...
                    >
                      ...
                      {title.isUnlocked && !prefersReducedMotion && (title.rarity === 'rare' || title.rarity === 'epic' || title.rarity === 'legendary') && isSelected && (
                        <>
                          {[...Array(6)].map((_, i) => (
                            <motion.div
                              key={i}
                    ...
                    {isSelected && title.isUnlocked && !prefersReducedMotion && (
                      <motion.div
                        className="absolute inset-0 rounded-full"
                        animate={{
                          scale: [1, 1.15, 1],
                ...
                <div className="flex items-center justify-center md:justify-start gap-2">
                  <span 
                    ...
                    style={{
                      ...
                      color: getBadgeGeometry(selectedTitle.rarity, selectedTitle.title).ringColor
                    }}
                  >
                ...
                {selectedTitle.isUnlocked && (
                  <>
                    <motion.button
                      ...
                      whileTap={{ scale: 0.95 }}
                      onClick={handleEquip}
                      disabled={equipping || isAnimating || selectedTitle.achievementId === equippedAchievementId}
                      ...
                      style={{
                        boxShadow: selectedTitle.achievementId === equippedAchievementId 
                          ? 'none' 
      ...
      <AnimatePresence>
        {showShareMenu && selectedTitle && (
          ...
          >
            <p className="text-xs mb-3 text-center font-medium" style={{ color: getBadgeGeometry(selectedTitle.rarity, selectedTitle.title).ringColor }}>
 ...
 */
function EquipAnimation({ title, onComplete }: { title: TitleData; onComplete: () => void }) {
  const [phase, setPhase] = useState<'intro' | 'particle-burst' | 'badge-form' | 'shine' | 'title-reveal' | 'complete'>('intro');
  const [prefersReducedMotion, setPrefersReducedMotion] = useState(false);
  ...
  const config = rarityConfig[title.rarity as keyof typeof rarityConfig] || rarityConfig.common;
  ...
  useEffect(() => {
    ...
    const timeline = [
      ...
      { phase: 'badge-form' as const, delay: introDuration + particleBurstDuration },
      { phase: 'shine' as const, delay: introDuration + particleBurstDuration + badgeFormDuration },
      { phase: 'title-reveal' as const, delay: introDuration + particleBurstDuration + badgeFormDuration + shineDuration },
      { phase: 'complete' as const, delay: introDuration + particleBurstDuration + badgeFormDuration + shineDuration + titleRevealDuration }
    ];
    ...
    const completeTimer = setTimeout(onComplete, 
      introDuration + particleBurstDuration + badgeFormDuration + shineDuration + titleRevealDuration + completeDuration
    );
  ...
  const phaseIndex = ['intro', 'particle-burst', 'badge-form', 'shine', 'title-reveal', 'complete'].indexOf(phase);
              ...
              {[0, 1].map((i) => (
                <motion.div
                  ...
                  style={{
                    ...
                    borderRadius: title.rarity === 'common' || title.rarity === 'legendary' ? '50%' : '20%',
                    clipPath: badgeConfig.clipPath
                  }}
                />
              ))}
