...
export function detectCloudflareError(error: any): CloudflareErrorResult {
  ...
  if (isCloudflareHTML) {
    ...
    // ✅ Handle Supabase "Internal server error" responses (database overload or temporary issues)
    userMessage = 'Database temporarily overloaded';
    technicalMessage = 'Supabase internal server error (database may be experiencing high load)';
    shouldRetry = true;
    retryAfterMs = 3000; // Wait a bit longer for database to recover
  ...
  try {
    ...
    if (!options.quiet && !(error as any).isHttp2) {
      ...
      const isNetworkError = 
        ...
        errorMsg.includes('Internal server error') || // ✅ Treat internal server errors as network errors
        errorMsg.includes('Internal Server Error');
      
      if (isNetworkError) {
        // Only log once per key to reduce spam - use minimal logging
        if (!key.includes('lock')) { // Don't log lock errors at all
          console.warn(`⚠️ DB error for \"${key}\": ${errorMsg.substring(0, 80)}`);
        }
      } else {
