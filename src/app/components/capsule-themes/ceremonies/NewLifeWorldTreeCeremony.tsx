/**
 * New Life - SUNRISE SYMPHONY: Epic Mountain Sunrise (LEGENDARY)
 * Performance-safe: CSS keyframes for loops, useMemo for all random data,
 * capped element counts, no repeat:Infinity on JS animations.
 */

import React, { useState, useEffect, useMemo } from 'react';
import { motion, AnimatePresence } from 'motion/react';
import confetti from 'canvas-confetti';

interface NewLifeWorldTreeCeremonyProps {
  capsuleTitle: string;
  media?: any[];
  isPreview?: boolean;
  onComplete?: () => void;
}

const WORLD_CSS = `
@keyframes wt-pop-ring {
  0%   { transform: translate(-50%,-50%) scale(0); opacity: 0.92; }
  55%  { opacity: 0.6; }
  100% { transform: translate(-50%,-50%) scale(4.4); opacity: 0; }
}
@keyframes wt-flash {
  0%   { opacity: 0; }
  8%   { opacity: 1; }
  40%  { opacity: 0; }
}
@keyframes wt-orb-float {
  0%   { transform: translate(0, 0) scale(1); opacity: 0.9; }
  100% { transform: translate(var(--dx), -90px) scale(0.4); opacity: 0; }
}
@keyframes star-twinkle {
  0%,100% { opacity: 0.18; transform: scale(0.9); }
  50%      { opacity: 0.7;  transform: scale(1.15); }
}
@keyframes blade-sway {
  0%,100% { transform: rotate(-3deg); }
  50%      { transform: rotate(3deg); }
}
@keyframes sun-disc-pulse {
  0%,100% { transform: scale(1); }
  50%      { transform: scale(1.055); }
}
@keyframes sun-ray-pulse {
  0%,100% { opacity: 0.75; }
  50%      { opacity: 1; }
}
@keyframes corona-pulse {
  0%,100% { transform: translate(-50%,-50%) scale(1); }
  50%      { transform: translate(-50%,-50%) scale(1.09); }
}
@keyframes wish-ambient {
  0%,100% { opacity: 0.38; }
  50%      { opacity: 0.52; }
}
`;

export function NewLifeWorldTreeCeremony({
  capsuleTitle,
  media = [],
  isPreview = false,
  onComplete
}: NewLifeWorldTreeCeremonyProps) {
  const [stage, setStage] = useState<'predawn' | 'firstlight' | 'rays' | 'painting'>('predawn');

  const isMobile = typeof window !== 'undefined' && window.innerWidth < 768;
  const wtColors = useMemo(() => ['#fbbf24','#fb923c','#f97316','#fde68a','#ffffff','#10b981','#fcd34d','#fef9c3'], []);
  const wtFwPositions = useMemo(() => [
    {x:8,y:12},{x:22,y:8},{x:38,y:15},{x:52,y:6},{x:68,y:14},{x:82,y:10},{x:14,y:25},{x:90,y:20},
  ].slice(0, isMobile ? 5 : 8), [isMobile]);
  const wtFwSparks = useMemo(() => wtFwPositions.map(() =>
    Array.from({length: isMobile ? 14 : 20}, (_, i) => {
      const a = (i / (isMobile ? 14 : 20)) * Math.PI * 2;
      const d = 50 + (i % 5) * 20;
      return { x: Math.cos(a)*d, y: Math.sin(a)*d, color: wtColors[i % wtColors.length], delay: i*0.04 };
    })
  ), [wtFwPositions, wtColors, isMobile]);
  const wtFwRings = useMemo(() => wtFwPositions.map(() =>
    Array.from({length: 3}, (_, i) => ({ delay: i*0.15, color: ['#fbbf24','#fb923c','#fde68a'][i] }))
  ), [wtFwPositions]);
  const wtOrbs = useMemo(() => Array.from({length: isMobile ? 10 : 18}, (_, i) => ({
    x: 5 + (i * 5.5) % 90, dx: (i % 7 - 3) * 18, dur: 2.5 + (i % 4) * 0.5,
    delay: i * 0.18, color: wtColors[i % wtColors.length]
  })), [wtColors, isMobile]);

  useEffect(() => {
    const timeline = [
      { time: 0,     action: () => setStage('predawn') },
      { time: 4000,  action: () => setStage('firstlight') },
      { time: 8000,  action: () => setStage('rays') },
      { time: 13000, action: () => setStage('painting') },
      { time: 22500, action: () => onComplete?.() }
    ];
    const timeouts = timeline.map(({ time, action }) => setTimeout(action, time));
    const failsafe = setTimeout(() => onComplete?.(), 23000);
    return () => { timeouts.forEach(clearTimeout); clearTimeout(failsafe); };
  }, []);

  useEffect(() => {
    if (stage !== 'painting') return;
    const colors = ['#fbbf24','#fb923c','#f97316','#fde68a','#ffffff','#10b981','#fcd34d'];
    const base = { spread: 80, ticks: 200, gravity: 0.9, decay: 0.93, startVelocity: 38, colors };
    confetti({ ...base, particleCount: isMobile ? 70 : 120, angle: 60, origin: { x: isMobile ? 0.12 : 0, y: 0.7 } });
    confetti({ ...base, particleCount: isMobile ? 70 : 120, angle: 120, origin: { x: isMobile ? 0.88 : 1, y: 0.7 } });
    if (!isMobile) {
      const t1 = setTimeout(() => confetti({ ...base, particleCount: 80, angle: 90, origin: { x: 0.5, y: 0.6 } }), 380);
      const t2 = setTimeout(() => {
        confetti({ ...base, particleCount: 100, angle: 60, origin: { x: isMobile ? 0.12 : 0, y: 0.65 } });
        confetti({ ...base, particleCount: 100, angle: 120, origin: { x: isMobile ? 0.88 : 1, y: 0.65 } });
      }, 950);
      return () => { clearTimeout(t1); clearTimeout(t2); };
    }
  }, [stage]);

  const isLit      = stage !== 'predawn';
  const isRadiant  = stage === 'rays' || stage === 'painting';
  const isPainting = stage === 'painting';

  /* ── Stable random data ── */
  const bgStars = useMemo(() => Array.from({ length: 80 }, (_, i) => ({
    size: Math.random() * 2.5 + 0.8,
    left: Math.random() * 100,
    top:  Math.random() * 60,
    duration: 2 + Math.random() * 1.5,
    delay: i * 0.022
  })), []);

  const grassBlades = useMemo(() => Array.from({ length: 60 }, (_, i) => ({
    x:    (i / 60) * 100,
    y:    Math.random() * 35,
    h:    12 + Math.random() * 18,
    initRot: (Math.random() - 0.5) * 8,
    swayDur: 2.5 + Math.random() * 1.5,
    delay: 0.5 + i * 0.02
  })), []);

  const midFlowers = useMemo(() => {
    const types = ['🌸', '🌺', '🌼', '🌻', '🌷', '🦋', '🌸', '🌺', '🦋', '🌼'];
    return Array.from({ length: 40 }, (_, i) => ({
      emoji: types[i % types.length],
      x: (i / 40) * 100,
      y: 5 + Math.random() * 26,
      size: 11 + Math.random() * 10,
      delay: 0.6 + i * 0.03
    }));
  }, []);

  const fgFlowers = useMemo(() => {
    const types = ['🌸', '🌺', '🌼', '🌻', '🌷'];
    return Array.from({ length: 20 }, (_, i) => ({
      emoji: types[i % types.length],
      x: (i / 20) * 100,
      y: Math.random() * 7,
      size: 18 + Math.random() * 14,
      delay: 1.2 + i * 0.05
    }));
  }, []);

  const godRays = useMemo(() => Array.from({ length: 24 }, (_, i) => ({
    angle: -70 + i * 6,
    height: 1000 + (i % 4) * 150,
    intensity: 0.4 + (1 - Math.abs(i - 12) / 12) * 0.4,
    delay: i * 0.1
  })), []);

  const atmosRays = useMemo(() => Array.from({ length: 12 }, (_, i) => ({
    angle: -60 + i * 10,
    height: 700 + (i % 4) * 100,
    delay: 1.5 + i * 0.15
  })), []);

  const dustParticles = useMemo(() => Array.from({ length: 18 }, (_, i) => ({
    x: 45 + (i / 18) * 10,
    y: 40 + (i % 9) * 4,
    xA: Math.sin(i) * 15,
    xB: Math.cos(i) * 10,
    dur: 8 + (i % 4),
    delay: 2 + (i % 5) * 0.4
  })), []);

  return (
    <div className="relative w-full h-full overflow-hidden bg-black flex items-center justify-center">
      <style>{WORLD_CSS}</style>

      {/* Sky */}
      <motion.div
        className="absolute inset-0 z-0"
        animate={{
          background: isPainting
            ? 'linear-gradient(to bottom, #93c5fd 0%, #60a5fa 8%, #fbbf24 22%, #f97316 36%, #fb7185 52%, #ec4899 66%, #c026d3 80%, #7e22ce 100%)'
            : isRadiant
            ? 'linear-gradient(to bottom, #bfdbfe 0%, #93c5fd 10%, #fcd34d 24%, #fbbf24 38%, #fb923c 52%, #f97316 66%, #c026d3 82%, #581c87 100%)'
            : isLit
            ? 'linear-gradient(to bottom, #1e3a5f 0%, #2d5986 18%, #f97316 38%, #fbbf24 55%, #fde68a 72%, #fef9c3 100%)'
            : 'linear-gradient(to bottom, #020617 0%, #0f172a 25%, #1e1b4b 60%, #312e81 85%, #3730a3 100%)'
        }}
        transition={{ duration: 4, ease: [0.4, 0, 0.2, 1] }}
      />

      {/* Atmospheric glow */}
      {isRadiant && (
        <motion.div
          className="absolute inset-0 z-1 pointer-events-none"
          style={{
            background: 'radial-gradient(ellipse at 50% 60%, rgba(251,191,36,0.3) 0%, rgba(251,146,60,0.2) 30%, transparent 60%)',
            mixBlendMode: 'overlay'
          }}
          initial={{ opacity: 0 }}
          animate={{ opacity: [0, 0.8, 0.6] }}
          transition={{ duration: 3.5, delay: 1, ease: 'easeOut' }}
        />
      )}

      {/* Stars — CSS twinkle, capped at 80 */}
      <AnimatePresence>
        {(stage === 'predawn' || stage === 'firstlight') && (
          <motion.div className="absolute inset-0 z-5" exit={{ opacity: 0 }} transition={{ duration: 2 }}>
            {bgStars.map((s, i) => (
              <div
                key={`star-${i}`}
                className="absolute rounded-full bg-white"
                style={{
                  left: `${s.left}%`,
                  top:  `${s.top}%`,
                  width:  `${s.size}px`,
                  height: `${s.size}px`,
                  boxShadow: `0 0 ${s.size * 2.5}px rgba(255,255,255,0.8)`,
                  animation: `star-twinkle ${s.duration}s ease-in-out ${s.delay}s infinite`
                }}
              />
            ))}
          </motion.div>
        )}
      </AnimatePresence>

      {/* Mountains */}
      <motion.div
        className="absolute z-10"
        style={{ left: 0, right: 0, bottom: '35%', height: '26%', display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}
        initial={{ opacity: 0 }}
        animate={{ opacity: isLit ? 1 : 0.3 }}
        transition={{ duration: 3, ease: [0.4, 0, 0.2, 1] }}
      >
        {isPainting && [0,1,2,3,4].map(i => (
          <motion.div key={`valley-mist-${i}`} style={{
            position: 'absolute', left: `${15 + i * 18}%`, bottom: 0,
            width: '120px', height: '35%',
            background: 'linear-gradient(to top, rgba(251,191,36,0.25), rgba(251,191,36,0.15) 40%, transparent)',
            filter: 'blur(25px)', mixBlendMode: 'overlay'
          }}
          initial={{ opacity: 0 }} animate={{ opacity: [0, 0.7, 0.5] }}
          transition={{ duration: 2.5, delay: 2 + i * 0.3, ease: 'easeOut' }}
          />
        ))}
        {[
          { left: '5%',  height: '80%',  width: '18%', peakOffset: '48%' },
          { left: '18%', height: '95%',  width: '21%', peakOffset: '52%' },
          { left: '35%', height: '110%', width: '24%', peakOffset: '50%' },
          { left: '55%', height: '92%',  width: '20%', peakOffset: '47%' },
          { left: '72%', height: '98%',  width: '22%', peakOffset: '51%' }
        ].map((peak, i) => (
          <div key={i} style={{
            position: 'absolute', left: peak.left, bottom: 0,
            width: peak.width, height: peak.height,
            clipPath: `polygon(${peak.peakOffset} 0%, 0% 100%, 100% 100%)`,
            background: isLit
              ? i % 2 === 0
                ? 'linear-gradient(125deg, #5f27cd 0%, #341f97 50%, #2c3a47 100%)'
                : 'linear-gradient(125deg, #341f97 0%, #2c3a47 50%, #1e272e 100%)'
              : 'linear-gradient(125deg, #0f172a 0%, #020617 50%, #000000 100%)',
            filter: isRadiant ? 'brightness(1.3) saturate(1.4)' : 'brightness(0.85)'
          }} />
        ))}
        {isPainting && [1,2,4].map((peakIndex) => {
          const peaks = [
            { left: '8%', height: '75%', width: '15%', peakOffset: '48%' },
            { left: '20%', height: '88%', width: '18%', peakOffset: '52%' },
            { left: '35%', height: '100%', width: '20%', peakOffset: '50%' },
            { left: '52%', height: '85%', width: '17%', peakOffset: '47%' },
            { left: '66%', height: '92%', width: '19%', peakOffset: '51%' },
            { left: '82%', height: '78%', width: '16%', peakOffset: '49%' }
          ];
          const peak = peaks[peakIndex];
          return (
            <motion.div key={`snow-${peakIndex}`} style={{
              position: 'absolute', left: peak.left, bottom: peak.height,
              width: peak.width, height: '18%',
              clipPath: `polygon(${peak.peakOffset} 0%, 30% 80%, 70% 80%)`,
              background: 'linear-gradient(125deg, #ffffff, #f8fafc, #e2e8f0)',
              filter: 'brightness(1.4) drop-shadow(0 2px 8px rgba(255,255,255,0.5))'
            }}
            initial={{ opacity: 0, y: 10 }} animate={{ opacity: 0.95, y: 0 }}
            transition={{ delay: 1.2 + peakIndex * 0.25, duration: 1.8 }}
            />
          );
        })}
      </motion.div>

      {/* Ground */}
      <motion.div className="absolute z-18" style={{ left: 0, right: 0, bottom: 0, height: '38%' }}
        initial={{ opacity: 0 }}
        animate={{
          opacity: 1,
          background: isPainting
            ? 'linear-gradient(to bottom, rgba(250,204,21,0.95) 0%, rgba(234,179,8,1) 45%, rgba(202,138,4,1) 100%)'
            : isLit
            ? 'linear-gradient(to bottom, rgba(202,138,4,0.7) 0%, rgba(161,98,7,0.85) 45%, rgba(133,77,14,1) 100%)'
            : 'linear-gradient(to bottom, rgba(15,23,42,0.5) 0%, rgba(14,61,33,0.7) 45%, rgba(7,30,16,1) 100%)'
        }}
        transition={{ duration: 2.5 }}
      />

      {/* Grass blades — CSS sway, capped at 60 */}
      <AnimatePresence>
        {isLit && (
          <>
            {grassBlades.map((b, i) => (
              <motion.div
                key={`grass-${i}`}
                className="absolute z-19"
                style={{
                  left: `${b.x}%`, bottom: `${b.y}%`,
                  width: '2.5px', height: `${b.h}px`,
                  background: isPainting
                    ? 'linear-gradient(to top, rgba(22,163,74,0.8), rgba(34,197,94,0.5), transparent)'
                    : 'linear-gradient(to top, rgba(21,128,61,0.6), rgba(22,163,74,0.4), transparent)',
                  borderRadius: '50%', transformOrigin: 'bottom center'
                }}
                initial={{ scaleY: 0, opacity: 0 }}
                animate={{ scaleY: [0, 1.1, 1], opacity: [0, 0.8], rotate: [b.initRot] }}
                transition={{ duration: 1.2, delay: b.delay, ease: 'easeOut' }}
              >
                <div style={{ width: '100%', height: '100%', animation: `blade-sway ${b.swayDur}s ease-in-out ${b.delay}s infinite` }} />
              </motion.div>
            ))}
          </>
        )}
      </AnimatePresence>

      {/* Mid flowers — capped at 40 */}
      <AnimatePresence>
        {isPainting && (
          <>
            {midFlowers.map((f, i) => (
              <motion.div
                key={`plain-flower-${i}`}
                className="absolute z-20"
                style={{ left: `${f.x}%`, bottom: `${f.y}%`, fontSize: `${f.size}px` }}
                initial={{ scale: 0, opacity: 0 }}
                animate={{ scale: 1, opacity: 1 }}
                exit={{ scale: 0, opacity: 0 }}
                transition={{ duration: 0.7, delay: f.delay, ease: [0.22, 1, 0.36, 1] }}
              >
                {f.emoji}
              </motion.div>
            ))}
          </>
        )}
      </AnimatePresence>

      {/* Sun */}
      <AnimatePresence>
        {isLit && (
          <motion.div
            className="absolute z-14"
            style={{ left: '50%', bottom: '58%', transform: 'translateX(-50%)' }}
            initial={{ y: 200, scale: 0.95, opacity: 0 }}
            animate={{ y: isRadiant ? -15 : 80, scale: isRadiant ? 1.25 : 0.95, opacity: 1 }}
            transition={{ duration: stage === 'firstlight' ? 5 : 6, ease: [0.25, 0.46, 0.45, 0.94] }}
          >
            {/* Sun disc — CSS pulse */}
            <div style={{
              width: '300px', height: '300px', borderRadius: '50%',
              background: isRadiant
                ? 'radial-gradient(circle at center, #ffffff 0%, #fffff0 10%, #fffde7 20%, #fff9c4 32%, #fef3c7 44%, #fde68a 58%, #fbbf24 74%, #f59e0b 88%, #f97316 100%)'
                : 'radial-gradient(circle at center, #ffffff 0%, #fff9c4 22%, #fde68a 46%, #fbbf24 70%, #f59e0b 88%, #f97316 100%)',
              boxShadow: isRadiant
                ? '0 0 0 18px rgba(251,191,36,0.35), 0 0 0 40px rgba(251,146,60,0.2), 0 0 100px 50px rgba(251,191,36,0.75), 0 0 200px 80px rgba(251,146,60,0.45)'
                : '0 0 0 12px rgba(251,191,36,0.28), 0 0 70px 35px rgba(251,191,36,0.6)',
              animation: isRadiant ? 'sun-disc-pulse 3.5s ease-in-out infinite' : 'none'
            }} />

            {/* 12 sun rays — single opacity wrapper + CSS pulse */}
            <motion.div
              className="absolute inset-0 pointer-events-none"
              animate={{ opacity: isRadiant ? [0.75, 1, 0.75] : [0.5, 0.7, 0.5] }}
              transition={{ duration: 2.8, ease: 'easeInOut' }}
            >
              {[...Array(12)].map((_, i) => (
                <div key={`ray-${i}`} style={{
                  position: 'absolute', left: '50%', top: '50%',
                  width: isRadiant ? '200px' : '100px',
                  height: isRadiant ? '8px' : '5px',
                  marginTop: isRadiant ? '-4px' : '-2.5px',
                  background: 'linear-gradient(to right, rgba(255,255,255,0.75), rgba(251,191,36,0.5) 30%, transparent)',
                  transformOrigin: '0% 50%',
                  transform: `rotate(${i * 30}deg)`,
                  borderRadius: '0 4px 4px 0',
                  animation: `sun-ray-pulse ${2.8 + i * 0.1}s ease-in-out infinite`
                }} />
              ))}
            </motion.div>

            {/* Corona halo — CSS pulse */}
            <div style={{
              position: 'absolute', left: '50%', top: '50%',
              width: isRadiant ? '420px' : '340px',
              height: isRadiant ? '420px' : '340px',
              transform: 'translate(-50%,-50%)',
              borderRadius: '50%',
              background: isRadiant
                ? 'radial-gradient(circle at center, transparent 40%, rgba(251,191,36,0.18) 55%, rgba(251,146,60,0.1) 72%, transparent 88%)'
                : 'radial-gradient(circle at center, transparent 44%, rgba(251,191,36,0.14) 58%, transparent 80%)',
              animation: isRadiant ? 'corona-pulse 4s ease-in-out infinite' : 'none',
              pointerEvents: 'none'
            }} />

            {/* Bloom */}
            {isRadiant && (
              <motion.div style={{
                position: 'absolute', left: '50%', top: '50%',
                width: '700px', height: '700px',
                transform: 'translate(-50%,-50%)', borderRadius: '50%',
                background: 'radial-gradient(circle at center, rgba(251,191,36,0.45) 0%, rgba(251,146,60,0.28) 35%, rgba(249,115,22,0.12) 62%, transparent 80%)',
                filter: 'blur(55px)', pointerEvents: 'none'
              }}
              initial={{ scale: 0, opacity: 0 }}
              animate={{ scale: [0, 1.7, 1.4], opacity: [0, 0.95, 0.8] }}
              transition={{ duration: 3.5, ease: 'easeOut' }}
              />
            )}
          </motion.div>
        )}
      </AnimatePresence>

      {/* God rays — entrance only, no repeat */}
      <AnimatePresence>
        {(stage === 'rays' || isPainting) && (
          <>
            <motion.div className="absolute inset-0 z-16 pointer-events-none" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1 }}>
              {godRays.map((r, i) => (
                <motion.div key={`godray-${i}`} style={{
                  position: 'absolute', left: '50%', bottom: '58%',
                  width: '18px', height: `${r.height}px`,
                  background: `linear-gradient(to top, rgba(251,191,36,${r.intensity}), rgba(251,191,36,${r.intensity * 0.65}) 40%, rgba(251,191,36,${r.intensity * 0.3}) 70%, transparent)`,
                  transformOrigin: 'bottom center',
                  transform: `translateX(-50%) rotate(${r.angle}deg)`,
                  filter: 'blur(20px)', mixBlendMode: 'screen'
                }}
                initial={{ scaleY: 0, opacity: 0 }}
                animate={{ scaleY: [0, 1.15, 1], opacity: [0, 1, 0.85] }}
                transition={{ duration: 3, delay: r.delay, ease: 'easeOut' }}
                />
              ))}
            </motion.div>

            {isPainting && (
              <motion.div className="absolute inset-0 z-15 pointer-events-none" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1 }}>
                {atmosRays.map((r, i) => (
                  <motion.div key={`atmos-${i}`} style={{
                    position: 'absolute', left: '50%', bottom: '58%',
                    width: '20px', height: `${r.height}px`,
                    background: 'linear-gradient(to top, rgba(255,243,199,0.2), rgba(255,243,199,0.1) 40%, transparent)',
                    transformOrigin: 'bottom center',
                    transform: `translateX(-50%) rotate(${r.angle}deg)`,
                    filter: 'blur(30px)', mixBlendMode: 'overlay'
                  }}
                  initial={{ scaleY: 0, opacity: 0 }}
                  animate={{ scaleY: [0, 1.05, 1], opacity: [0, 0.6, 0.5] }}
                  transition={{ duration: 3, delay: r.delay, ease: 'easeOut' }}
                  />
                ))}
              </motion.div>
            )}

            {/* Dust particles — no repeat */}
            {isPainting && dustParticles.map((d, i) => (
              <motion.div key={`dust-${i}`} className="absolute z-17 rounded-full"
                style={{
                  left: `${d.x}%`, top: `${d.y}%`,
                  width: '3px', height: '3px',
                  background: 'rgba(251,243,199,0.8)',
                  boxShadow: '0 0 8px rgba(251,191,36,0.6)',
                  filter: 'blur(1px)'
                }}
                initial={{ opacity: 0, y: 0 }}
                animate={{ opacity: [0, 0.8, 0.6, 0.8], y: [-30, 15, -20], x: [0, d.xA, d.xB] }}
                transition={{ duration: d.dur, delay: d.delay, ease: 'easeInOut' }}
              />
            ))}
          </>
        )}
      </AnimatePresence>

      {/* Couple silhouette */}
      <motion.div
        className="absolute z-32"
        style={{ left: '30%', bottom: '14%', display: 'flex', gap: '4px', alignItems: 'flex-end' }}
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: isLit ? 1 : 0.25, y: 0 }}
        transition={{ duration: 3, delay: 2, ease: [0.4, 0, 0.2, 1] }}
      >
        <svg width="42" height="100" viewBox="0 0 42 100" style={{ filter: isLit ? 'drop-shadow(0 0 22px rgba(0,0,0,0.95))' : 'none' }}>
          <circle cx="21" cy="13" r="11" fill={isLit ? '#0f172a' : '#020617'} />
          <ellipse cx="21" cy="9" rx="11" ry="8" fill={isLit ? '#0f172a' : '#020617'} />
          <path d="M 10,10 Q 10,6 12,5" stroke={isLit ? '#0f172a' : '#020617'} strokeWidth="3" fill="none" />
          <path d="M 32,10 Q 32,6 30,5" stroke={isLit ? '#0f172a' : '#020617'} strokeWidth="3" fill="none" />
          <rect x="17" y="22" width="8" height="7" fill={isLit ? '#0f172a' : '#020617'} />
          <path d="M 7,29 L 35,29 L 35,48 L 31,68 L 26,80 L 16,80 L 11,68 L 7,48 Z" fill={isLit ? '#0f172a' : '#020617'} />
          <path d="M 35,33 Q 46,35 52,40" stroke={isLit ? '#0f172a' : '#020617'} strokeWidth="6" fill="none" strokeLinecap="round" />
          <path d="M 7,33 Q 2,48 2,55" stroke={isLit ? '#0f172a' : '#020617'} strokeWidth="5.5" fill="none" strokeLinecap="round" />
          <rect x="11" y="80" width="7" height="20" rx="1" fill={isLit ? '#0f172a' : '#020617'} />
          <rect x="24" y="80" width="7" height="20" rx="1" fill={isLit ? '#0f172a' : '#020617'} />
        </svg>
        <svg width="40" height="92" viewBox="0 0 40 92" style={{ filter: isLit ? 'drop-shadow(0 0 22px rgba(0,0,0,0.95))' : 'none' }}>
          <circle cx="20" cy="12" r="9.5" fill={isLit ? '#0f172a' : '#020617'} />
          <ellipse cx="20" cy="16" rx="14" ry="20" fill={isLit ? '#0f172a' : '#020617'} />
          <path d="M 7,14 Q 5,24 6,32" stroke={isLit ? '#0f172a' : '#020617'} strokeWidth="5" fill="none" strokeLinecap="round" />
          <path d="M 33,14 Q 35,24 34,32" stroke={isLit ? '#0f172a' : '#020617'} strokeWidth="5" fill="none" strokeLinecap="round" />
          <rect x="16" y="20" width="8" height="6" fill={isLit ? '#0f172a' : '#020617'} />
          <path d="M 11,26 L 29,26 L 27,42 L 25,50 L 15,50 L 13,42 Z" fill={isLit ? '#0f172a' : '#020617'} />
          <path d="M 15,50 L 25,50 L 32,80 L 8,80 Z" fill={isLit ? '#0f172a' : '#020617'} />
          <line x1="13" y1="50" x2="27" y2="50" stroke={isLit ? '#1e293b' : '#020617'} strokeWidth="1.5" />
          <path d="M 11,30 Q 3,34 -8,40" stroke={isLit ? '#0f172a' : '#020617'} strokeWidth="5" fill="none" strokeLinecap="round" />
          <path d="M 29,30 Q 34,42 36,52" stroke={isLit ? '#0f172a' : '#020617'} strokeWidth="4.5" fill="none" strokeLinecap="round" />
          <ellipse cx="16" cy="82" rx="4" ry="6" fill={isLit ? '#0f172a' : '#020617'} />
          <ellipse cx="24" cy="82" rx="4" ry="6" fill={isLit ? '#0f172a' : '#020617'} />
        </svg>
      </motion.div>

      {/* Foreground flowers — capped at 20, no blur */}
      <AnimatePresence>
        {isPainting && (
          <>
            {fgFlowers.map((f, i) => (
              <motion.div
                key={`fg-flower-${i}`}
                className="absolute z-38"
                style={{ left: `${f.x}%`, bottom: `${f.y}%`, fontSize: `${f.size}px` }}
                initial={{ scale: 0, opacity: 0 }}
                animate={{ scale: 1, opacity: 1 }}
                exit={{ scale: 0, opacity: 0 }}
                transition={{ duration: 0.8, delay: f.delay, ease: [0.22, 1, 0.36, 1] }}
              >
                {f.emoji}
              </motion.div>
            ))}
          </>
        )}
      </AnimatePresence>

      {/* Firework clusters — painting finale */}
      <AnimatePresence>
        {isPainting && (
          <>
            {wtFwPositions.map((pos, pi) => (
              <React.Fragment key={`wt-fw-${pi}`}>
                {wtFwSparks[pi].map((s, si) => (
                  <motion.div key={`wt-spark-${pi}-${si}`} className="absolute z-51 rounded-full"
                    style={{ left: `${pos.x}%`, top: `${pos.y}%`, width: 6, height: 6, background: s.color }}
                    initial={{ x: 0, y: 0, scale: 0, opacity: 0 }}
                    animate={{ x: s.x, y: s.y, scale: [0,1.4,0], opacity: [0,1,0] }}
                    transition={{ duration: 1.2, delay: s.delay, ease: 'easeOut' }}
                  />
                ))}
                {wtFwRings[pi].map((r, ri) => (
                  <div key={`wt-ring-${pi}-${ri}`} className="absolute rounded-full border-2"
                    style={{
                      left: `${pos.x}%`, top: `${pos.y}%`, width: 20, height: 20,
                      borderColor: r.color, animation: `wt-pop-ring 0.9s ease-out ${r.delay}s both`
                    }}
                  />
                ))}
                <div key={`wt-flash-${pi}`} className="absolute rounded-full"
                  style={{
                    left: `${pos.x}%`, top: `${pos.y}%`, width: 40, height: 40,
                    background: `radial-gradient(circle, ${wtColors[pi % wtColors.length]}cc, transparent)`,
                    filter: 'blur(8px)', animation: 'wt-flash 0.5s ease-out both'
                  }}
                />
              </React.Fragment>
            ))}
            {wtOrbs.map((orb, i) => (
              <div key={`wt-orb-${i}`} className="absolute rounded-full z-49"
                style={{
                  left: `${orb.x}%`, bottom: '22%', width: 10, height: 10,
                  background: orb.color, boxShadow: `0 0 14px ${orb.color}`,
                  '--dx': `${orb.dx}px`,
                  animation: `wt-orb-float ${orb.dur}s ease-out ${orb.delay}s both`
                } as React.CSSProperties}
              />
            ))}
          </>
        )}
      </AnimatePresence>

      {/* Title */}
      <AnimatePresence>
        {isPainting && (
          <motion.div
            className="absolute text-center z-50"
            style={{ left: '50%', top: '10%', transform: 'translateX(-50%)' }}
            initial={{ opacity: 0, y: -50 }}
            animate={{ opacity: [0, 1], y: [-50, 0] }}
            transition={{ duration: 2.5, delay: 2, ease: [0.4, 0, 0.2, 1] }}
          >
            <motion.h2
              className="text-3xl md:text-4xl font-black"
              style={{
                background: 'linear-gradient(135deg, #fbbf24, #ffffff, #fb923c, #fbbf24)',
                backgroundSize: '200% 200%',
                WebkitBackgroundClip: 'text',
                WebkitTextFillColor: 'transparent',
                filter: 'drop-shadow(0 0 30px rgba(251,191,36,0.8))'
              }}
              animate={{ backgroundPosition: ['0% 50%', '100% 50%', '0% 50%'] }}
              transition={{ duration: 3, repeat: 5 }}
            >
              🌅 Every Sunrise Is A New Beginning 🌅
            </motion.h2>
            <motion.p className="text-lg md:text-xl font-medium mt-2"
              style={{ color: '#fde68a', textShadow: '0 0 20px rgba(251,191,36,0.9), 0 4px 14px rgba(0,0,0,0.9)' }}
              initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              transition={{ delay: 3.5, duration: 2 }}
            >
              A new life · a new horizon ✨
            </motion.p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
  );
}
