...
import { X, Share2, Crown, Sparkles, Star, Zap, Sunrise, Link2, Check } from 'lucide-react';
...
export function TitleRewardModalEnhanced({ 
  ...
  const [animationPhase, setAnimationPhase] = useState<'entry' | 'reveal' | 'glow' | 'burst' | 'idle' | 'complete'>('entry');
  ...
  const handleClose = React.useCallback(() => {
    console.log('👑 [Title Modal] Closing - emitting titleModalClosed event', { title, rarity, achievementName });
    
    // Emit custom event with title details
    window.dispatchEvent(new CustomEvent('titleModalClosed', {
      detail: { 
        title: title,
        rarity: rarity,
  ...
  const getRarityConfig = (rarity: string) => {
    const configs = {
      ...
      legendary: {
        ...
        gradient: 'from-rose-500 via-orange-400 via-yellow-300 via-cyan-400 to-pink-400',
        ...
        lightRays: 12,
        badge: 'eclipse',
        bgGradient: 'from-rose-600 via-orange-500 via-yellow-400 via-cyan-500 to-pink-500'
      }
    };
  ...
  const getShareText = (platform: 'twitter' | 'facebook' | 'linkedin' | 'generic') => {
    const baseUrl = 'https://www.erastimecapsule.com';
    const rarityEmoji = {
      common: '⚪',
      uncommon: '🟢',
      rare: '🟣',
  ...
  useEffect(() => {
    if (isOpen && title) {
      console.log('🌅 [Horizon Chromatic Fusion v4.0] Celebration:', title, rarity);
  ...
  const getBadgeClipPath = (badge: string) => {
    const paths = {
      ...
      star12: 'polygon(50% 0%, 61% 35%, 98% 35%, 68% 57%, 79% 91%, 50% 70%, 21% 91%, 32% 57%, 2% 35%, 39% 35%)',
  ...
  const titleGradient = titleColors.length >= 2 
          ...
          >
            <motion.div
              ...
              animate={{ 
                ...
                y: 0,
                rotateX: 0,
                x: prefersReducedMotion ? 0 : shakeIntensity > 0 ? [0, -shakeIntensity, shakeIntensity, -shakeIntensity, 0] : 0
              }}
            ...
            >
              ...
              <div className="absolute inset-0 opacity-20" style={{ overflow: 'hidden', pointerEvents: 'none' }}>
                {!prefersReducedMotion && config.lightRays > 0 && [...Array(config.lightRays)].map((_, i) => {
              ...
              <div className="relative flex flex-col items-center text-center px-6 md:px-12 py-12 md:py-20">
                ...
                >
                  ...
                  >
                    <span className="text-xl md:text-2xl text-white/90 drop-shadow-lg">{titleIcon}</span>
                    <span 
                ...
                >
                  ...
                  <div 
                    ...
                    style={{
                      ...
                      boxShadow: '0 20px 60px rgba(0,0,0,0.5), inset 0 0 40px rgba(255,255,255,0.3)'
                    }}
                  >
                    ...
                    {(rarity === 'epic' || rarity === 'legendary') && !prefersReducedMotion && (
                      <motion.div
                        className="absolute inset-0"
                        animate={{
                          rotate: [0, 360]
                  ...
                  {!prefersReducedMotion && animationPhase !== 'entry' && (
                    <>
                      {[...Array(rarity === 'legendary' ? 20 : rarity === 'epic' ? 16 : 12)].map((_, i) => (
                        <motion.div
                          key={i}
                ...
                >
                  Unlocked by achieving <span className="font-semibold text-white">{achievementName}</span>
                </motion.p>
                ...
                >
                  Activate this title in the Horizon Gallery to showcase your legacy
                </motion.p>
